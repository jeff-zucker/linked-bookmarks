export class OpenCultureBrowser {
  constructor(){
  ...
  async init(){
    for(let containerElm of document.getElementsByClassName('ocb')){
      ...
      try {
        ...
        await this.getTopic(subject,containerElm,'start')
        document.getElementById('ocbStart').click();
      }
      catch(e){ alert(e) }
    }
  }
  async getTopic(topic,containingElement,start){
    const self=this
    ...
    if(subTopics.length>0){
      ...
      for(let b in bookmarks){
        ...
        bookElement.addEventListener("click", async function() {
          document.getElementById(self.bookmarksDisplay).src=bookmarkLink.uri
        });
...
window.addEventListener('DOMContentLoaded', async (event) => {
  const ocb = new OpenCultureBrowser()
  ocb.init()
})
